...
class Office extends \yii\db\ActiveRecord
{
	...
	public function rules()
	{
		return [
			...
			[['name'], 'required'],
			[['name'], 'string', 'max' => 45],
			[['register_address', 'bank_name', 'telephone','bank_code', 'payment_account', 'fax'], 'string'],
			['uploadedFile', 'file'],
			[['chief_name', 'glbuh_name', 'check_buh_name', 'unp', 'okpo'], 'integer'],
			[['name'], 'string', 'max' => 200],
			[['logo'], 'string'],
			[['email'], 'string', 'max' => 45]
		];
...
//					$this->logo = $filePath;
	...
	public function getSignPerson()
	{
		...
		if ($signDoc) {
			return Employee::find()->where(['id' => $signDoc])->one();
		} else {
			return false;
		}
