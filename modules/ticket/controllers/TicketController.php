...
//            ],
    ...
    {
        $request = Yii::$app->request->get('SearchTicket');
        $contract = Contract::findOne(['id' => isset($request['contract_id']) ? $request['contract_id'] : null]);
...
//        $tickets = $searchModel->search(Yii::$app->request->queryParams)->getModels();
        ...
        $calculation = new CalculationModel([
            ...
            'actDate' => isset($request['actDate']) ? $request['actDate'] : null,
        ]);
...
//		$calculation->closed_to_date = ;
        ...
        echo $this->renderAjax('_act_layout',
            ['model' => $calculation, 'contract' => $contract]);
        $html = ob_get_contents();
        ob_end_clean();
        $pdf->WriteHTML($html);
        $filename = 'act-' . $calculation->closed_at . '-' . $calculation->closed_to_date . '.pdf';
